...
module.exports.Create = function (mymap, API) {
    ...
    const Initialized: Promise<void> = new Promise<void>(resolveInitialized => {
                    ...
                    if (break_feature_set) {
                        ...
                        if (ownership == "WARDENS" && warden_features.length > 0) {
                            WardenRoutes.features.push({
                                ...
                                geometry: {type: "LineString", coordinates: warden_features}
                            });
                            warden_features = [];
                        }

                        if (last_ownership == "COLONIALS" && colonial_features.length > 0) {
                            ColonialRoutes.features.push({
                                type: "Feature",
                                properties: Paths.features[i].properties,
                                geometry: {type: "LineString", coordinates: colonial_features}
                            });
                            colonial_features = [];
                        }
                    }
            ...
            for (var k = 1; k < layer._latlngs.length; k++) {
                ...
                if (lat != null && lng != null && lat2 != null && lng2 != null) {
                    ...
                    ControlLayer.addRoad([[lat, lng], [lat2, lng2]], {
                        control: control == "COLONIALS" ? 0 : (control == "WARDENS" ? 1 : (control == "OFFLINE" ? 2 : 3)),
                        tier: tier
                    });
                }
            }
        ...
        var FoxholeRouter = {
            ...
            setRoute: (route) => {
                FoxholeRouter.currentRoute = route;
            },
            ...
            drawRouteLine: function (ctx) {
                ...
                if (this.Control.routeSelected != null && waypoint_count > 1) {
                    ...
                    for (let style of [
                        ...
                        {
                            color: this.Control.routeSelected.name == "Shortest Route" ? "#9E3031" : "#5E9339",
                            opacity: 1,
                            weight: 2
                        }]) {
                    ...
                    for (var i = 0; i < crossroads.length - 1; i++) {
                        const j = crossroads[i];
                        {
                            ...
                            instructions.push({
                                ...
                                time: 0,
                                text: j.region.concat('|').concat(text).concat('|').concat(border.toString()).concat('|').concat(region_change ? '1' : '0').concat('|').concat(turnicon).concat('|').concat(j.tier)
                            });
                        }
                    }
                    instructions.push({
