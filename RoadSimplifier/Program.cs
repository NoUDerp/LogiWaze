...
var roads = await JsonSerializer.DeserializeAsync<Roads>(File.OpenRead("Roads.json"));
var buckets = new System.Collections.Concurrent.ConcurrentDictionary<Properties, List<Feature>>();
foreach (var feature in roads.features)
    buckets.GetOrAdd(feature.properties, f => []).Add(feature);

var tolerance = 4;
foreach (var bucket in buckets.Values)
    for (var i = 0; i < bucket.Count; i++)
    {
        var feature = bucket[i];
        var points = feature.geometry.coordinates.Select(p => new Vector2(p[0], p[1])).ToList();
        var distance = CatmullRomSpline.CalculateCumulativeDistances(points).SkipLast(1).Sum();
        var n = (int)Math.Max(2, Math.Round(distance / tolerance));
        var result = CatmullRomSpline.GenerateSplineCurve(points, n, feature.geometry.coordinates.Length);
        // determine angle between points[i-1], points[i], and points[i+1]
        // determine length between points[i-1] and points[i+1]
        // 
        bucket[i] = feature with
        {
            geometry = feature.geometry with { coordinates = result.Select(p => new[] { p.X, p.Y }).ToArray() }
        };
    }

roads = roads with {  features = buckets.Values.SelectMany(b => b.ToArray()).ToArray() };

await using var file = File.OpenWrite("Roads_output.json");
